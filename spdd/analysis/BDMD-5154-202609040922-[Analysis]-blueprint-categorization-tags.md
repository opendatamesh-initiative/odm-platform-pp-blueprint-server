...
- **Amend existing `V2__blueprint_labels.sql`** — do **not** add a new Flyway version. Label migrations have not been pushed; work is still local.
...
- Agent: no new `/labels` paths. Existing viewer/editor handlers remain sufficient. Additive field on `LabelRes` only.
...
- **Label (catalog)**: First-class categorization label with its own identity. Holds **name**, **description**, **color**, and audit timestamps. Shared vocabulary, not owned by one blueprint. Unused labels (zero assignments) are valid. No seed rows. Distinct from the Git release tag on a blueprint version. Physical table `labels` (Flyway `V2__blueprint_labels.sql`); entity `Label`; resource `LabelRes`; search options today are **name `LIKE` only**.
...
- **Editor authorization (boundary, not this service)**: `blindata-agent` already has viewer/editor path handlers for `/labels` (`BLUEPRINTS_EDITOR` writes; viewer reads). Blueprint-server itself does not check roles. **No new agent paths** for grouping.
...
- **Ownership:** Editors own catalog labels (including the optional group string) and assignments. The catalog is shared. Enforcement of Editor vs Viewer is agent-side (existing `/labels` handlers).
...
- **Label name:** required; case-insensitive uniqueness mirroring blueprints (`existsByNameIgnoreCase` / exclude self on update) → `ResourceConflictException` **(409)** on duplicate, same as blueprint names. **Global** — group is not part of the key. Two labels cannot share a name even in different groups. **Only simple characters**, plus length/required: those field rules stay `BadRequestException` **(400)** via `validate()`, same as other catalog fields.
...
- Authorization: Editors only for catalog and assignment writes — **existing handlers in** `blindata-agent`. No new `/labels` paths.
...
Keep the existing GitHub-style label taxonomy in this service and **add optional group metadata on the catalog label**, analog of custom-property grouping. Current local artifacts (`Label`, `LabelRes`, `V2__blueprint_labels.sql`, `LabelServiceImpl.validate`, `LabelController` OpenAPI sort list, `LabelControllerIT`) already implement the catalog **without** `group`. This increment extends those artifacts; it does not add a new aggregate, controller, or use case.
...
`blueprints` is existing (relationship context). Tables: `labels` (amend V2), `blueprints_labels` (unchanged). There is **no** group table and **no** FK from labels to a group identity.
...
- **Character class:** Only simple characters on label `name`. Invalid → 400 via existing `validate()` / `BadRequestException`. Exact allowed set is already specified in the REASONS prompt. **Do not apply that pattern to `group`.**
...
- **Amend V2, do not add V3.** Label migrations have not been pushed; add `label_group` on the existing `labels` table definition in `V2__blueprint_labels.sql`.
...
- **No seed labels.**
- **Authorization:** Existing agent `/labels` viewer/editor handlers. **No new paths.** Additive field only.
...
| New Flyway V3 for `label_group`                           | Rejected: labels not pushed; amend V2                                        |
...
- **Illegal characters in label name:** 400. **Spaces in group:** allowed (e.g. `Status`).
...
- **Flyway + JPA validate:** Amended `labels.label_group` in V2 must match `@Column(name = "label_group")` on `Label`. Environments that already applied the previous V2 checksum need a local schema recreate/repair.
...
- **Agent auth:** Existing `/labels` handlers suffice. Additive JSON field does not require new routes. Until handlers existed this was a gap; they now exist in `blindata-agent` as a companion delivery.
...
| 5   | Provide basic / seed labels                       | N/A          | **Decided: none.**                                                                                                 |
...
| 10  | Unique label names + simple characters            | Yes          | Case-insensitive uniqueness like blueprints → **409**. Illegal characters / required / length → **400**.           |
| 11  | No duplicate label on one blueprint               | Yes          | Composite PK of the join; validate request collections.                                                            |
| 12  | Editor-only authorization                         | Companion    | Same `BLUEPRINTS_EDITOR` / `VIEWER` handlers in `blindata-agent`. No new paths for grouping.                        |
| 13  | UI list/detail chips and filter                   | Follow-up    | API returns labels on `BlueprintRes`; UI grouping presentation is the UI SPDD.                                     |
| 14  | Optional denormalized `group` on label            | Yes          | Column `label_group`; API/Java/JSON `group`. Amend V2; no V3.                                                      |
...
| 20  | Agent: no new paths                               | Yes          | Additive field only; existing viewer/editor `/labels` handlers.                                                    |
