...
- **Instantiation pipeline (monorepo, no composition)**: `InstantiateBlueprintVersion` loads the blueprint version, validates manifest and parameters, initializes Git, clones sources and targets, runs `renderAndCopy` on the templating port, then `commitAndPush` per target path — the extension point for “after template instantiated, before push” sits between templating completion and Git push (or equivalently as a final templating phase before leaving the working tree).
...
| 6   | Instantiate use case: parse descriptor after templating, add metadata, then push | Yes          | Fits between `renderAndCopy` and `commitAndPush` in `InstantiateBlueprintVersion`; may warrant a small dedicated outbound port for testability.          |
