...
- hexagonal SoC: auth headers factory→git port only; author defaults inside git port; descriptor enrichment via templating outbound port; both **instantiate** and **update** orchestrate Git workflows via **granular** per-use-case git ports so steps stay readable (inlined in `instantiateMonorepoNoComposition` / `updateMonorepoNoComposition`); `CreatePullRequest` stays inside update `openPullRequest` only. Layout is selected via shared **`InstantiationScenario`** (only monorepo-no-composition is implemented). Templating ports expose **`monorepoNoCompositionRenderAndCopy`** and delegate to shared **`BlueprintRenderService`**.
...
- **Pure Initial Checkpoint Commit (orphan / isolated)**: For repos that already contain user files (Scenario 1), the first checkpoint must be an **orphan/isolated commit** containing **only** rendered blueprint files — never the tip of `main` that already mixes user content. That pure commit is tagged, then **merged into main**, so pre-existing user files remain “additions on main” relative to the baseline. Orchestrated in instantiate inside **`instantiateMonorepoNoComposition`** using granular git port ops (`withClonedSourceAndTarget`, orphan/commit/tag/merge/push); tag/orphan names from `BlueprintGitNamingConventions`.
...
- **Update Data Product from Blueprint Version (use case)**: Orchestration for Step 2 (+ optional Step 3 PR): resolve current/next versions → `InstantiationScenario` switch → `updateMonorepoNoComposition` (singular ROOT; Git steps inlined) → optional **`openPullRequest`** (option A; `GitException` → **`warnings`**) → return **`results`** / **`warnings`**.
- **Granular update Git outbound port**: `withClonedSourceAndTargetAtCheckpoint`, `createAndCheckoutBranch`, `cleanWorkingTreePreservingGit` (port-local NIO clean preserving `.git`), `commitAll`, `createCheckpointTag`, `pushBranch`, `pushTag`, plus `openPullRequest(...)` (git-utils `CreatePullRequest` only inside the port).
- **Shared render utility**: `BlueprintRenderService` (`@Service` under `...services.usecases`) owns **`monorepoNoCompositionRenderAndCopy`**; instantiate/update templating port impls delegate. `BlueprintDataProductDescriptorService` lives in the same shared `usecases` package (moved out of `instantiate`).
...
- **Working-tree Clean before Re-instantiate**: Explicit clean before writing the new blueprint render so removed template files do not linger — performed as use-case-orchestrated `gitPort.cleanWorkingTreePreservingGit` inside `updateMonorepoNoComposition`.
...
- Git orchestration is **use-case-visible with granular outbound ports**, with git-utils used only inside port impls (except update catches git-utils `GitException` when mapping PR open failures to **`warnings`**):
  - Update: inlined in `updateMonorepoNoComposition` (clone at checkpoint → create/checkout update branch → clean → `monorepoNoCompositionRenderAndCopy` → enrich → commit → tag → pushBranch + pushTag) → optional separate **`openPullRequest`** when global flag is true (option A).
...
- **Separate update use case vs extending instantiate**: Git flows differ → **Dedicated update use case**; evolve instantiate for pure checkpoint + SoC alignment (auth off command, enrichment via templating port, scenario routing, Initial Generation inlined in `instantiateMonorepoNoComposition` with granular git port).
...
- **Git outbound port altitude**: → both update and instantiate use **granular** git ports so workflows stay in the use case (inlined scenario methods rather than separate `updateFromCheckpoint` / `createInitialCheckpoint` named methods); update keeps separate **`openPullRequest`** (`CreatePullRequest` only inside the port).
- **PR open after update (option A)**: → Global flag; after successful update Git steps, use case calls `openPullRequest`. If PR fails (`GitException`) → still HTTP 200 with **`warnings`** (update acknowledged); PR target = entry `pullRequestTargetBranch` or repo `defaultBranch`.
- **git-utils consumption**: → Consume APIs **only inside git port impls** for Git ops; avoid embedding raw JGit/provider HTTP in blueprint-server use cases. Update use case may catch `GitException` solely to convert PR open failures into **`warnings`**.
- **Pure initial commit for non-empty repos (Scenario 1)**: → instantiate Initial Generation with orphan → tag → merge → push; tag/orphan names from `BlueprintGitNamingConventions`.
- **Empty vs non-empty target at first apply**: Prefer the pure-commit path consistently.
- **Working-tree clean on update**: → Use-case-orchestrated `cleanWorkingTreePreservingGit` inside `updateMonorepoNoComposition` (port-local NIO delete preserving `.git`; stage deletions on commit).
- **Author defaults**: → Inside git port commit paths when name/email blank (`BlueprintGitNamingConventions` defaults: `odm-blueprint-server` / `odm-blueprint-server@local`).
- **Auth headers**: → Factory → git port only; not on domain command; not validated in use case `execute()`.
- **Descriptor enrichment**: → Templating outbound port method; not a Spring bean dependency of the use case. `BlueprintDataProductDescriptorService` relocated to shared `...services.usecases`.
- **Shared render**: → `BlueprintRenderService.monorepoNoCompositionRenderAndCopy` (not a generic `renderAndCopy` name).
...
- **Remaining work is blueprint-server orchestration only**: Wire granular ports to git-utils 1.1.0; no further Git-library feature work required for BDMD-5127.
...
- **Scope of Initial Generation / Scenario 1**: **In this ticket** — evolve instantiate with Initial Generation inlined in `instantiateMonorepoNoComposition` + granular git port + `InstantiationScenario` routing + SoC alignment (auth off command, enrichment via templating port).
...
- **Current vs next version validation** (ordering, adjacency, lineage match) — still open beyond same-Blueprint existence checks (implemented: same Blueprint UUID required; current ≠ next).
- **Who performs the “Merge into main” of the pure initial commit**: **Chosen default** — server-side local merge via use-case-orchestrated `gitPort.mergeBranch` inside Initial Generation (then push the integration branch).
...
- **Dependency version bump**: blueprint-server adopts git-utils **1.1.0** (done).
- **Instantiate SoC + pure checkpoint**: Implemented via auth off command, enrichment via templating port, `InstantiationScenario`, and Initial Generation + granular git port alongside update.
- **Clean + render correctness**: Bad clean (touching `.git`, missing deletion staging) corrupts merge semantics — owned by port-local `cleanWorkingTreePreservingGit` / `commitAll` inside `updateMonorepoNoComposition`; mitigate with ITs for Scenario 1 and 2 (`TagBasedThreeWayMergeIT`).
- **Local merge then push of integration branch**: After `mergeBranch`, push integration branch; mid-flow failure can leave remote without merge while tag was pushed — ordering is explicit in instantiate Initial Generation (tag → merge → pushBranch → pushTag).
...
- **Controller IT coverage**: `BlueprintUpdateDataProductControllerIT` covers happy paths, PR warnings, key validations, missing checkpoint, branch collision, auth failure; full Gherkin matrix (composition/polyrepo `NotSupported`, null parameters, next-tag collision, etc.) is partially covered — extend ITs as needed.
...
| 10  | Initial generation produces a **pure** checkpoint (Scenario 1): orphan → tag → merge into main when user files may exist                 | Yes          | Instantiate Initial Generation + granular git port; covered by `TagBasedThreeWayMergeIT`  |
| 11  | Pre-existing user files are not deleted by later blueprint updates (Scenario 1 outcome)                                                  | Yes          | Guaranteed by pure baseline; verified with IT                                    |
| 12  | Same-file different-line user vs blueprint edits auto-merge in PR (Scenario 2A)                                                          | Yes          | Covered by `TagBasedThreeWayMergeIT`; server does not resolve                                          |
| 13  | Same-line overlapping edits produce PR conflict for user resolution (Scenario 2B)                                                        | Yes          | Covered by `TagBasedThreeWayMergeIT`; no server auto-resolve                                                                        |
...
| 15  | Hexagonal SoC: no auth headers / `CreatePullRequest` in use case; granular git ports; enrichment via templating port; both use cases aligned | Yes       | Synced REASONS canvas; use case may catch `GitException` for PR→warnings only       |

---

**Analysis notes:** Synced to the shipped blueprint-server implementation via `/spdd-sync` (`update-data-product`, shared `BlueprintRenderService` / `BlueprintGitNamingConventions` / `InstantiationScenario`, instantiate Initial Generation alignment, `git-utils` 1.1.0).
