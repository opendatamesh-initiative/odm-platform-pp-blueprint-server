...
- Scope is **odm-platform-pp-blueprint-server only**. Do not implement UI. Do not add new `blindata-agent` `/labels` paths (existing viewer/editor handlers remain sufficient; `group` is an additive field on `LabelRes`). No seed labels. No deprecation. No concatenated tags column. No new Flyway version after V2 for this column.
...
4. Validation and errors:
   ...
   - Label **name** required, max 255, **simple characters** `^[A-Za-z0-9][A-Za-z0-9_-]*$` → 400 on fail.
...
5. Persistence / Flyway:
   - **Amend existing** `V2__blueprint_labels.sql` beside `V1__init_schema_template.sql`. Do **not** add `V3` (or any later version) for `label_group`. Label migrations have not been pushed; work is still local.
   - Flyway already targets `hibernate.default_schema`.
   - JPA `ddl-auto: validate` must match the amended migration (`labels.label_group`).
...
6. Out of this slice:
   ...
   - New agent `/labels` paths. Agent `BLUEPRINTS_EDITOR` / `VIEWER` handlers for `/labels` ship in the **same delivery** as the UI (documented in the UI analysis); grouping does not add routes.
   - New Flyway version after V2 for this column.
...
### Amend Flyway migration - `V2__blueprint_labels.sql`

1. Responsibility: Create catalog and join tables in `src/main/resources/db/migration/postgresql/`, same style as `V1__init_schema_template.sql` (`create table if not exists`, `varchar(36)` ids, no business unique indexes). **Amend this existing V2 file in place.** Do **not** add `V3__*` (or any later version) for grouping.
...
2. `LabelRes` extends `VersionedRes`: `uuid`, `name`, `description`, `color`, optional `group` with `@Schema`. `@Schema(name = "labels")`. Schema text for `group`: optional catalog group (free text; presentation metadata; not a filter key). Values are trimmed on write; whitespace-only is stored as no group. Distinct groups are exact trimmed strings.
...
5. `validate(Label)`:
   ...
   - name required (`validateRequired` same helper style as blueprint).
   - name length ≤ 255.
   - name must match `^[A-Za-z0-9][A-Za-z0-9_-]*$` else `BadRequestException` stating only simple characters (letters, digits, hyphen, underscore; must start with letter or digit).
   - color: if has text, length ≤ 32 and must match `^#[0-9A-Fa-f]{6}$` else 400.
   - group: optional. **Trim** first and **write the trimmed value back** on the entity (empty after trim → null or empty string, consistent with omit). If no text after trim, do not throw required. If text remains, length ≤ 255 else 400. Do **not** apply the name character pattern. Internal spaces allowed. Do not uniqueness-check group. Distinct stored values are exact trimmed strings (no case-folding).
...
2. `BlueprintSearchOptions`: `List<String> labelUuids` with `@Parameter` describing “Filter blueprints that have any of these label UUIDs (match any). Repeat the query parameter for multiple values.”
...
2. `validate`: after existing field checks, `validateLabels(blueprint)`:
   ...
   - duplicate uuids in the collection (case-sensitive uuid string) → `BadRequestException` that a blueprint cannot have the same label twice.
...
7. Do not add a new Flyway version after V2 for `label_group`.
...
1. Functional constraints:
   ...
   - Catalog `group` is optional presentation metadata; omit/null/blank after trim is valid; whitespace-only is trimmed to no group; UI **"Other"** is not stored by this API. Distinct groups are exact trimmed strings (custom-properties pattern).
...
5. Business rule constraints:
   ...
   - Label name simple characters and color `#RRGGBB` when set (400).
   - Label group optional free text max 255 **after trim**; not name-pattern; not required; not unique. Trim on write; whitespace-only is not stored. Distinct groups are exact trimmed strings (no case-folding).
...
7. Technical constraints:
   ...
   - Column `label_group` / field `group`. Amend V2; **no V3** for this column.
...
8. Data constraints: ids `varchar(36)`; name/color/group lengths as columns; color optional; group optional (trimmed; empty after trim stored as no group).
9. API constraints:
...
10. Out of scope: create-on-apply, RESTRICT delete, deprecation, seed data, concatenated `CONTAINS` on blueprints, join entity, first-class groups, exclusivity, per-group name uniqueness, search-by-group, filter-by-group, new agent routes, new Flyway version after V2. UI grouping presentation is the companion UI SPDD (not generated from this server prompt). Agent `/labels` handlers are the same-delivery companions (not generated from this server prompt).
