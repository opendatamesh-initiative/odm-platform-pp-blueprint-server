...
2. Technical implementation:
   ...
   - Registry (odm-platform-pp-registry-server): keep `dataProductRepo` 1:1. Add `DataProductAdditionalRepo` table (cascade from `DataProduct`; FK to the product only—**no** DB unique constraint on `(data_product_uuid, repository_key)`). Expose on `DataProductRes` as an optional list. Uniqueness of `repositoryKey` within a product’s additional repos is enforced in the **DataProduct core service** (root aggregate `validate` / create-overwrite hooks), not by a fixed schema constraint. Instantiate does **not** call registry. CRUD/mapping follows registry generic CRUD patterns.
...
6. Registry `DataProductAdditionalRepo` is a JPA entity; `DataProduct` remains `VersionedEntity`. Additional repos cascade via `DataProduct` mapping. Uniqueness of `repositoryKey` per product is validated on the root aggregate in `DataProductsServiceImpl` (or equivalent Generic CRUD `validate` / `beforeCreation` / `beforeOverwrite`), not via a JPA `@UniqueConstraint` or Flyway unique index.
...
2. Attributes:
   - `uuid`: String — PK
   - `repositoryKey`: String — `targetRepositories[].key` (not the root row)
...
3. Mapping: `DataProduct` `@OneToMany` additional repos; cascade persist/delete with product; FK `data_product_uuid` only. **Do not** add a DB unique constraint, unique index, or JPA `@UniqueConstraint` / `@Table(uniqueConstraints=…)` on `(data_product_uuid, repository_key)`.
4. Domain validation (root aggregate): In the DataProduct **core service** (`DataProductsServiceImpl` `validate` and create/overwrite hooks), reject duplicate `repositoryKey` values within the same product’s `additionalDataProductRepos` list (case rules consistent with other registry string keys). Surface as the registry’s usual bad-request / conflict mapping. This is the **only** uniqueness enforcement for additional-repo keys in this ticket.
...
6. Tests: `DataProductControllerIT` create/update/read with extras; duplicate `repositoryKey` rejected by the core service (not by a DB constraint violation); root pointer unchanged.
...
Feature: Registry stores additional keyed repositories
  ...
  I want to persist extra Git remotes with repository keys
...
| Registry additional repos                                         | `DataProductControllerIT` (registry-server)                                   | `whenCreateDataProductWithAdditionalRepositoriesThenReturnCreatedWithExtras`; `whenCreateDataProductWithDuplicateRepositoryKeyThenReturnBadRequest`                                                                                                                                                                                                                                                                                                                                    |
...
9. CRUD (`spdd/norms/GENERIC-CRUD-GUIDELINES.md`): Additional repositories cascade on `DataProduct` (same style as nested `dataProductRepo`). Enforce unique `repositoryKey` per product in the DataProduct core service `validate` / `beforeCreation` / `beforeOverwrite` — **not** with a Flyway unique index or JPA unique constraint. Blueprint-server instantiate does **not** use Generic CRUD for Git work.
